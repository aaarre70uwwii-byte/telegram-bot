...
# ----------------- الإعدادات الأساسية -----------------
DEVELOPER_ID = 7488375443 # أيدي المطور الخاص بك (ثابت ومحمي)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id == DEVELOPER_ID:
        await update.message.reply_text("👋 مرحباً بك يا مطور 𝐓𝐢𝐚! لوحة التحكم مفعّلة 👇", reply_markup=get_dev_keyboard())
    else:
        conn = sqlite3.connect('bot_data.db')
        cursor = conn.cursor()
        cursor.execute("SELECT value FROM settings WHERE key='welcome_text'"); welcome = cursor.fetchone()
        cursor.execute("SELECT value FROM settings WHERE key='updates_channel'"); channel = cursor.fetchone()
        conn.close()
...
async def handle_dev_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    elif text in ["أضف رد عام", "مسح رد عام", "تغير أسم البوت", "أضف الترحيب نص+بصوره", "الاذاعه خاص+مجموعات", "تغير قناه البوت"]:
        actions = {"أضف رد عام": 'add_reply', "مسح رد عام": 'del_reply', "تغير أسم البوت": 'change_name', "أضف الترحيب نص+بصوره": 'change_welcome', "الاذاعه خاص+مجموعات": 'broadcast', "تغير قناه البوت": 'change_updates_channel'}
        context.user_data['action'] = actions[text]
        await update.message.reply_text("📝 ارسل المطلوب الآن...")
    elif text == "تغير المطور الاساسي":
        await update.message.reply_text("⚠️ لا يمكن تغيير المطور من البوت.\nروح Railway > Variables وغير OWNER_ID واعمل Redeploy")
    conn.commit()
    conn.close()
...
async def handle_inputs_and_replies(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id == DEVELOPER_ID:
        ...
        if action:
            ...
            if action == 'add_reply' and '=' in text:
                ...
                await update.message.reply_text("✅ تمت الاذاعة")
            elif action == 'change_name':
                cursor.execute("UPDATE settings SET value=? WHERE key='bot_name'", (text,))
                await update.message.reply_text(f"✏️ تم تغيير اسم البوت الى: {text}")
    ...
    conn.close()
    if reply: await update.message.reply_text(reply[0])
