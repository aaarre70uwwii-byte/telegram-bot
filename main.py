...
def group_commands(message):
    ...
    if text == "رفع ادمن" and is_admin(message):
        bot.reply_to(message, "✅ تم رفع العضو ادمن")
    elif text == "تنزيل ادمن" and is_admin(message):
        bot.reply_to(message, "✅ تم تنزيل العضو من الادمنية")
...
def user_msg(message):
    ...
    txt = escape_md(message.text or message.caption or f"[{message.content_type}]")
    name = escape_md(message.from_user.first_name)
    bot.send_message(ADMIN_ID, f"⊱ 📩 رسالة جديدة\n⊱ من: {name}\n⊱ الايدي: `{message.from_user.id}`\n⊱ {txt}")
