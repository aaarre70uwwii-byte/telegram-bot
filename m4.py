...
def register_m4_handlers(bot):
    ...
    def fun_commands(m):
        ...
        if is_admin(bot, chat_id, user_id):
            ...
            elif txt == 'تعطيل اكتموه': ranks[chat_id]['disabled_aktmoh'] = True; save_data(); bot.reply_to(m, "❌ تم تعطيل اكتموه"); return
            elif txt == 'تفعيل اكتموه': ranks[chat_id]['disabled_aktmoh'] = False; save_data(); bot.reply_to(m, "✅ تم تفعيل اكتموه"); return
            elif txt == 'تعطيل زوجني': ranks[chat_id]['disabled_zawaj'] = True; save_data(); bot.reply_to(m, "❌ تم تعطيل الزواج"); return
            elif txt == 'تفعيل زوجني': ranks[chat_id]['disabled_zawaj'] = False; save_data(); bot.reply_to(m, "✅ تم تفعيل الزواج"); return
    ...
    def vote_mute(call):
        global vote_data # <-- هذا السطر اللي اضفناه
